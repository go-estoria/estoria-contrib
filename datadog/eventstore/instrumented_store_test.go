...
func TestNewInstrumentedStore(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{
			name:  "creates store with noop client",
			inner: inner,
			opts: []InstrumentedStoreOption{
				WithMeterProvider(&statsd.NoOpClient{}),
			},
			wantErr: false,
		},
		{
...
func TestInstrumentedStore_ReadStream_ErrorPropagated(t *testing.T) {
	...
	// CRITICAL REGRESSION TEST for Task 14 fix:
...
func TestInstrumentedStore_AppendStream_SpanName(t *testing.T) {
	...
	// We verified the code uses the correct constant ".AppendStream" in the implementation
...
func TestInstrumentedStoreOptions(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{
			name:                "defaults",
			opts:                []InstrumentedStoreOption{WithMeterProvider(&statsd.NoOpClient{})},
