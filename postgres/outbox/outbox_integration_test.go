...
func appendEvents(t *testing.T, es *pgeventstore.EventStore, streamID typeid.ID, events []*eventstore.WritableEvent) {
	t.Helper()
...
func newEventStore(t *testing.T, db *pgxpool.Pool, strat pgeventstore.Strategy, hooks ...pgeventstore.TransactionHook) *pgeventstore.EventStore {
	t.Helper()
...
// creating the DB schema and fataling the test on error.
func newOutbox(t *testing.T, db *pgxpool.Pool, handler pgoutbox.ItemHandler, opts ...pgoutbox.Option) *pgoutbox.Outbox {
	t.Helper()
	...
	for _, tt := range []struct {
	...
	}{
		{
			...
			run: func(t *testing.T, ctx context.Context, db *pgxpool.Pool, strat pgeventstore.Strategy) {
				...
				ob := newOutbox(t, db, collectingHandler(&mu, &received))
				es := newEventStore(t, db, strat, ob)

				streamID := typeid.NewV4("mystream")
				appendEvents(t, es, streamID, []*eventstore.WritableEvent{
		...
		},
		{
			...
			run: func(t *testing.T, ctx context.Context, db *pgxpool.Pool, strat pgeventstore.Strategy) {
				...
				appendEvents(t, es, streamA, []*eventstore.WritableEvent{{Type: "e", Data: []byte(`{"s":"A","n":1}`)}})
				appendEvents(t, es, streamB, []*eventstore.WritableEvent{{Type: "e", Data: []byte(`{"s":"B","n":1}`)}})
				appendEvents(t, es, streamC, []*eventstore.WritableEvent{{Type: "e", Data: []byte(`{"s":"C","n":1}`)}})
				appendEvents(t, es, streamA, []*eventstore.WritableEvent{{Type: "e", Data: []byte(`{"s":"A","n":2}`)}})
				appendEvents(t, es, streamB, []*eventstore.WritableEvent{{Type: "e", Data: []byte(`{"s":"B","n":2}`)}})
...
func TestOutbox_Run(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			appendEvents(t, es, streamID, events)
	...
	for _, tt := range []struct {
		...
		},
		{
			...
			run: func(t *testing.T, ctx context.Context, db *pgxpool.Pool, strat pgeventstore.Strategy) {
				...
				const maxRetries = 3
				ob := newOutbox(t, db, handler, pgoutbox.WithMaxRetries(maxRetries))
				es := newEventStore(t, db, strat, ob)
		...
		},
		{
			...
			run: func(t *testing.T, ctx context.Context, db *pgxpool.Pool, strat pgeventstore.Strategy) {
				...
				ob := newOutbox(t, db, handler, pgoutbox.WithMaxRetries(0))
				es := newEventStore(t, db, strat, ob)
		...
		},
		{
			...
			run: func(t *testing.T, ctx context.Context, db *pgxpool.Pool, strat pgeventstore.Strategy) {
				...
				streamA := typeid.NewV4("streamA")
				streamB := typeid.NewV4("streamB")
				appendEvents(t, es, streamA, []*eventstore.WritableEvent{
					{Type: "poison", Data: []byte(`{}`)},       // v1 — will be dead-lettered
					{Type: "after_poison", Data: []byte(`{}`)}, // v2 — must remain blocked
				})
				appendEvents(t, es, streamB, []*eventstore.WritableEvent{
					{Type: "ok1", Data: []byte(`{}`)},
					{Type: "ok2", Data: []byte(`{}`)},
				})
		...
		},
		{
			...
			run: func(t *testing.T, ctx context.Context, db *pgxpool.Pool, strat pgeventstore.Strategy) {
				...
				ob := newOutbox(t, db, handler, pgoutbox.WithMaxRetries(5))
				es := newEventStore(t, db, strat, ob)
	...
	t.Run("concurrent_processors_preserve_within_stream_order", func(t *testing.T) {
		...
		for v := 1; v <= eventsPerStream; v++ {
			for _, sid := range streams {
				appendEvents(t, es, sid, []*eventstore.WritableEvent{
	...
	for _, tt := range []struct {
		...
		},
		{
			...
			run: func(t *testing.T, ctx context.Context) {
				...
				appendEvents(t, es, streamX, writableX)
				appendEvents(t, es, streamY, writableY)
