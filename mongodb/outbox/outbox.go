...
// epoch is a sentinel "lease released / never leased" timestamp that always sorts before now.
...
func New(coll, streamState MongoCollection, handler ItemHandler, opts ...Option) (*Outbox, error) {
	...
	if streamState == nil {
		return nil, fmt.Errorf("stream state collection is required")
	}
	if handler == nil {
...
func (o *Outbox) HandleEvents(sessCtx context.Context, events []*eventstore.Event) error {
	...
	if _, err := o.streamState.UpdateOne(sessCtx,
		bson.D{{Key: "_id", Value: streamKey(streamType, streamID)}},
		bson.D{
			{Key: "$setOnInsert", Value: bson.D{
				{Key: "stream_type", Value: streamType},
				{Key: "stream_id", Value: streamID},
				{Key: "next_version", Value: minVersion},
				{Key: "leased_until", Value: epoch},
				{Key: "leased_by", Value: ""},
				{Key: "halted", Value: false},
			}},
			{Key: "$max", Value: bson.D{{Key: "last_version", Value: maxVersion}}},
		},
...
func (o *Outbox) EnsureIndexes(ctx context.Context) error {
	...
	streamModels := []mongo.IndexModel{
		{
			Keys:    bson.D{{Key: "leased_until", Value: 1}},
