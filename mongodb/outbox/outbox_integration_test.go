...
func drainProcessNext(ctx context.Context, t *testing.T, ob *mongooutbox.Outbox) {
	t.Helper()
	for {
...
func TestOutbox_Producer_WritesInAppendTxn(t *testing.T) {
	...
	h := newHarness(ctx, t, client, collectingHandler(&mu, &got), nil)

	streamID := typeid.NewV4("user")
	if _, err := h.store.AppendStream(ctx, streamID, writableEvents(3), es.AppendStreamOptions{}); err != nil {
		t.Fatalf("appending: %v", err)
	}

	if got := h.countOutbox(ctx, t); got != 3 {
...
func TestOutbox_Producer_RollbackOnAppendFailure(t *testing.T) {
	...
	var mu sync.Mutex
	var got []*mongooutbox.Item
	// A failing hook registered after the outbox forces the whole append (and the outbox
	// writes) to roll back.
	failHook := mongoeventstore.TransactionHookFunc(func(_ context.Context, _ []*es.Event) error {
		return errors.New("boom")
	})
	h := newHarness(ctx, t, client, collectingHandler(&mu, &got), nil, failHook)
	...
	if got := h.countOutbox(ctx, t); got != 0 {
...
func TestOutbox_ProcessNext_DeliversInOrderAndDeletes(t *testing.T) {
	...
	streamID := typeid.NewV4("user")
	if _, err := h.store.AppendStream(ctx, streamID, writableEvents(5), es.AppendStreamOptions{}); err != nil {
		t.Fatalf("appending: %v", err)
	}

	drainProcessNext(ctx, t, h.outbox)
...
func TestOutbox_PerStreamFIFO_ConcurrentWorkers(t *testing.T) {
	...
	h := newHarness(ctx, t, client, handler, []mongooutbox.Option{mongooutbox.WithPollInterval(20 * time.Millisecond)})
	...
	for v := range perStream {
		for _, s := range streams {
			if _, err := h.store.AppendStream(ctx, s, writableEvents(1), es.AppendStreamOptions{ExpectVersion: es.VersionPtr(int64(v))}); err != nil {
...
func TestOutbox_LeaseExpiry_Redelivery(t *testing.T) {
	...
	// The first delivery simulates a crashed/stuck worker: it signals, then blocks holding
	// the lease. Later deliveries (after the lease expires) record normally.
	handler := func(_ context.Context, _ *mongooutbox.Item) error {
	...
	h := newHarness(ctx, t, client, handler, leaseOpts)

	streamID := typeid.NewV4("user")
	if _, err := h.store.AppendStream(ctx, streamID, writableEvents(1), es.AppendStreamOptions{}); err != nil {
		t.Fatalf("appending: %v", err)
	}
...
func TestOutbox_RetryAndFail_HaltsStream(t *testing.T) {
	...
	h := newHarness(ctx, t, client, handler, []mongooutbox.Option{mongooutbox.WithMaxRetries(2)})

	streamID := typeid.NewV4("user")
	if _, err := h.store.AppendStream(ctx, streamID, writableEvents(2), es.AppendStreamOptions{}); err != nil {
		t.Fatalf("appending: %v", err)
	}
	...
	if got := h.countOutbox(ctx, t); got != 2 {
