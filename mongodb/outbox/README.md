...
- **Producer.** `Outbox` implements the event store's `TransactionHook`. On each append it inserts
  one outbox document per event **inside the append transaction**, so outbox items commit or roll
...
- **Consumer.** A polling loop claims work and processes it with **strict per-stream FIFO**:
  1. **Claim a stream** via an atomic `findOneAndUpdate` that grants a time-bounded *lease* on the
     stream's cursor (in the stream-state collection). The lease guarantees **at most one active
...
The outbox needs two collections, supplied by the caller. Name them with a leading underscore
(`_outbox`, `_outbox_streams`): typeid type names cannot begin with an underscore, so that
namespace is reserved for infrastructure collections, and the multi-collection strategy's event
collection enumeration ignores it. Without underscore names, a multi-collection store sharing the
database would sweep the outbox collections into its global reads.

## Usage

```go
db := client.Database("myapp")
outboxColl := db.Collection("_outbox")
streamColl := db.Collection("_outbox_streams")
...
strat, _ := strategy.NewSingleCollectionStrategy(db)
...
The outbox collections must belong to the same client as the event store so that producer writes
can participate in the append transaction; both storage strategies work.
