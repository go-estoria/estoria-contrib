...
func TestMultiCollectionStrategy_Integration_InsertStreamDocs(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{
			name:         "creates a new collection and inserts documents into it",
			haveSelector: strategy.CollectionSelectorFunc(func(streamID typeid.UUID) string { return "collectionA" }),
			haveExistingDocuments: map[string][]bson.M{
				"collectionA": {},
			},
		...
		{
			name:         "inserts documents into a non-empty collection with no matching stream type",
			haveSelector: strategy.CollectionSelectorFunc(func(streamID typeid.UUID) string { return "collectionA" }),
			haveExistingDocuments: map[string][]bson.M{
				"collectionA": {
					{"stream_id": "b610ff0b-5bb0-4e8f-9d2b-9cfb9818065d", "stream_type": "mockstreamtypeA", "data": "mockdata1", "offset": int64(1), "global_offset": int64(1)},
		...
		{
			name:         "inserts documents into a non-empty collection with a matching stream ID",
			haveSelector: strategy.CollectionSelectorFunc(func(streamID typeid.UUID) string { return "collectionA" }),
			haveExistingDocuments: map[string][]bson.M{
				"collectionA": {
					{"stream_id": "b610ff0b-5bb0-4e8f-9d2b-9cfb9818065d", "stream_type": "mockstreamtypeA", "data": "mockdata1", "offset": int64(1), "global_offset": int64(1)},
		...
		{
			name:         "inserts documents into a non-empty collection with a stream whose highest offset is equal to the global offset among multiple collections",
			haveSelector: strategy.CollectionSelectorFunc(func(streamID typeid.UUID) string { return "collectionA" }),
			haveExistingDocuments: map[string][]bson.M{
				"collectionA": {
					{"stream_id": "b610ff0b-5bb0-4e8f-9d2b-9cfb9818065d", "stream_type": "mockstreamtypeA", "data": "mockdata1", "offset": int64(1), "global_offset": int64(1)},
		...
		{
			name:         "inserts documents into a non-empty collection with a stream whose highest offset is less than the global offset among multiple collections",
			haveSelector: strategy.CollectionSelectorFunc(func(streamID typeid.UUID) string { return "collectionA" }),
			haveExistingDocuments: map[string][]bson.M{
				"collectionA": {
					{"stream_id": "b610ff0b-5bb0-4e8f-9d2b-9cfb9818065d", "stream_type": "mockstreamtypeA", "data": "mockdata1", "offset": int64(1), "global_offset": int64(1)},
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			haveStrategy, err := strategy.NewMultiCollectionStrategy(mongoClient, database, tt.haveSelector)
			if err != nil {
				t.Fatalf("unexpected error creating strategy: %v", err)
			}
