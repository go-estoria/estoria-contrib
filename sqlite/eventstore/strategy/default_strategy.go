...
// timestampFormat is the canonical format used to serialize event timestamps
// to and from SQLite TEXT columns. RFC3339 with nanosecond precision is
// lexicographically sortable and round-trips through time.Parse cleanly.
...
// The base query takes two parameters: ?=stream_type, ?=stream_id.
// When AfterVersion > 0, a third parameter is added for the version boundary.
func (s *DefaultStrategy) ReadStreamQuery(streamID typeid.ID, opts eventstore.ReadStreamOptions) (string, []any, error) {
	...
	return fmt.Sprintf(`
		...
		ORDER BY
			stream_offset %s
		%s
	`, quoteIdentifier(s.eventsTableName), versionClause, direction, limitClause),
		args, nil
}
...
func (s *DefaultStrategy) NextHighwaterMark(ctx context.Context, tx *sql.Tx, streamID typeid.ID, numEvents int) (int64, error) {
	...
	if err := tx.QueryRowContext(ctx, fmt.Sprintf(`
		...
		DO UPDATE SET last_offset = %s.last_offset + ?
		RETURNING last_offset`,
		quoteIdentifier(s.streamsTableName),
		quoteIdentifier(s.streamsTableName),
	), streamID.Type, streamID.UUID, numEvents, numEvents).Scan(&newOffset); err != nil {
...
func (s *DefaultStrategy) Schema() string {
	...
	`, quoteIdentifier(s.eventsTableName), quoteIdentifier(s.streamsTableName))
}
