...
func (s *EventStore) AppendStream(ctx context.Context, streamID typeid.ID, events []*eventstore.WritableEvent, opts eventstore.AppendStreamOptions) (_ []*eventstore.Event, retErr error) {
	...
	s.log.Debug("appending events to SQLite stream", "stream_id", streamID.String(), "events", len(events))
...
type StreamExistenceChecker interface {
	StreamExists(ctx context.Context, db *sql.DB, streamID typeid.ID) (bool, error)
}
...
func (s *EventStore) streamExists(ctx context.Context, streamID typeid.ID) (bool, error) {
	...
	exists, err := checker.StreamExists(ctx, s.db, streamID)
	if err != nil {
...
type StreamDeleter interface {
	DeleteStream(ctx context.Context, tx *sql.Tx, streamID typeid.ID, opts eventstore.DeleteStreamOptions) error
}
...
func (s *EventStore) DeleteStream(ctx context.Context, streamID typeid.ID, opts eventstore.DeleteStreamOptions) (retErr error) {
	...
	s.log.Debug("deleting events from SQLite stream", "stream_id", streamID.String(), "to_version", opts.ToVersion)
...
// isUniqueConstraintError reports whether err represents a SQLite UNIQUE constraint violation.
//
// SQLite drivers (modernc.org/sqlite, mattn/go-sqlite3) surface unique constraint
// violations through different concrete error types but consistently include
// "UNIQUE constraint failed" in the error message. Detecting via the message
// avoids importing — and depending on — a specific driver.
